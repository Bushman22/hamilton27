...
const CustomPrevArrow = (props) => {
    const { onClick } = props;
    return (
        <Button className='absolute right-1/2 -mr-8 transform -translate-x-1/2 bottom-[-50px] text-2xl cursor-pointer z-10 rounded-full bg-blue-300' onClick={onClick}>
...
const CustomNextArrow = (props) => {
    const { onClick } = props;
    return (
        <Button className='absolute left-1/2 transform -ml-8 translate-x-1/2 bottom-[-50px] text-2xl cursor-pointer z-10 rounded-full bg-blue-300' onClick={onClick}>
            <HiArrowSmallRight className="" />
        </Button>
    );
};

const HomeRoomsSlider = () => {
...
export default HomeRoomsSlider;
